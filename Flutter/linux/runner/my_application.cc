...
static void my_application_activate(GApplication* application) {
  ...
  gtk_window_set_title(window, "MuMuPai");
  gtk_window_set_default_size(window, 420, 720);

  // App-Icon setzen (aus dem Bundle)
  g_autoptr(GError) icon_error = NULL;
  gchar* exe_path = g_file_read_link("/proc/self/exe", NULL);
  if (exe_path) {
    gchar* exe_dir = g_path_get_dirname(exe_path);
    gchar* icon_path = g_build_filename(exe_dir, "data", "flutter_assets", "assets", "icon.png", NULL);
  ...
  gdk_rgba_parse(&background_color, "#0a0f0a");
